...
void save(int x)
{
    ...
    int i;
    if((fp=fopen("student.txt","rb+"))==NULL)/*以二进制读，写方式打开文件*/
    {
...
{
    FILE *fp;
    if((fp=fopen("student.txt","wb"))==NULL)/*以二进制写方式打开文件*/
    {
        printf("文件打开失败!\n");
        exit(1);
    }
...
int AddStudent(int n)
{
    char c='y';
    FILE *fp;
    if((fp=fopen("student.txt","ab"))==NULL)/*以二进制追加方式打开文件*/
    {
        printf("文件打开失败!\n");
        exit(1);
    }
...
/*查询一个学生信息**********************************************************************************/
void OutputOneStudent(int k)
{
    printf("学号:%d  姓名:%s  学院:%s  班级:%d\n",stu[k].number,stu[k].name,stu[k].xueyuan,stu[k].cla);
    printf("高数:%d  英语:%d  C语言:%d  总分:%d  平均分:%d\n",stu[k].score[k],stu[k].score[1],stu[k].score[2],stu[k].sum,stu[k].average);
}
...
void zfsc()
{
    ...
    if((wj=fopen("student1.dat","rb+"))==NULL)
    {
        printf("文件打开失败!\n");
       exit(1);
    } 
